...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.batch_size = train_batch_size #32
...
#손에 해당하는 vertex_id 들을 가져옴 guidance에 사용할듯? 
...
# sdf 표현, sdf 영역의 중심점(실세계 좌표의 ),전체 sdf의 규모 
    ...
    def load_and_freeze_clip(self, clip_version):
        ...
        return clip_model
    #텍스트를 clip으로 임베딩벡터로 만드는... 512벡터 
    def encode_text(self, raw_text):
        # raw_text - list (batch_size length) of strings with input text prompts
        device = next(self.clip_model.parameters()).device
        max_text_len = 30  # Specific hardcoding for the current dataset 허용하는 문장의 최대 토큰 수  받는건 30 이지만 최대 허용을 77이기에 30이하만 받고 나머지는 0으로 패딩  
 ...
 #모델이 맞추어야하는 부분을 알려주는 마스크 -> 첫프레임의 전체 마지막 프레임의 위치를 제외하면 나머지는 모델이 예측함 이부분들은 loss에서 제외
    def prep_start_end_condition_mask_pos_only(self, data, actual_seq_len):
        ...
        return mask 
 #웨이포인트 부분 마스크 -> 왜 필요하냐? 이 부분의 loss는 계산하지 않기 때문 왜? 입력이니까 모델의 결과가 아니니까   
...
#save_obj_only= True 하면 이미지랑 여러가지 저장안됨 동영상이랑 등등등
    ...
    #모델의 출력 결과를 바탕으로 사람(Human)과 객체(Object)의 3D 메쉬(Vertices + Joints)를 재구성하는 함수
